...
<div class="row inserted-item-in-list-<?= $ctr ?> selectedItemListDesign selectedItemInInputbox-<?= $ctr?> hidden">
	<?php if($type == '0'): ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-id-in-list-<?= $ctr ?>" value="0-<?= $services_id ?>" name="QuotationDetail[description][]" class="inputForm form-control partsservicesName" />		
		</div>
	...
	<?php else: ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-id-in-list-<?= $ctr ?>" value="1-<?= $parts_id ?>" name="QuotationDetail[description][]" class="inputForm form-control partsservicesName" />		
		</div>
	
	<?php endif; ?>

	<div class="col-md-3">
		<input type="text" id="parts-services-qty-in-list-<?= $ctr ?>" value="<?= $quantity ?>" name="QuotationDetail[quantity][]" onchange="updateSelectedSubtotal(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control partsservicesQty" />	
	</div>

	<div class="col-md-3">
		<input type="text" id="parts-services-price-in-list-<?= $ctr ?>" value="<?= $unit_price ?>" name="QuotationDetail[unit_price][]" onchange="updateSelectedSubtotal(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control partsservicesPrice" />	
	</div>
