...
<div class="row update-item-in-list-<?= $ctr ?> selectedItemListDesign selectedUpdateItemInInputbox-<?= $ctr?> hidden">
	<?php if($type == '0'): ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-update-id-in-list-<?= $ctr ?>" value="0-<?= $services_id ?>" name="InvoiceDetail[description][]" class="inputForm form-control update_partsservicesName" />		
		</div>
	...
	<?php else: ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-update-id-in-list-<?= $ctr ?>" value="1-<?= $parts_id ?>" name="InvoiceDetail[description][]" class="inputForm form-control update_partsservicesName" />		
		</div>
	
	<?php endif; ?>

	<div class="col-md-3">
		<input type="text" id="parts-services-update-qty-in-list-<?= $ctr ?>" value="<?= $quantity ?>" name="InvoiceDetail[quantity][]" onchange="editSelectedSubtotalInvoice(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control update_partsservicesQty" />	
	</div>

	<div class="col-md-3">
		<input type="text" id="parts-services-update-price-in-list-<?= $ctr ?>" value="<?= $unit_price ?>" name="InvoiceDetail[unit_price][]" onchange="editSelectedSubtotalInvoice(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control update_partsservicesPrice" />	
	</div>

	<div class="col-md-3">
		<input type="text" id="parts-services-update-subtotal-in-list-<?= $ctr ?>" value="<?= $sub_total ?>" name="InvoiceDetail[sub_total][]" class="inputForm inputboxTotalAlignment form-control update_partsservicesSubtotal" readonly="readonly" />	
	</div>
