	...
	<div class="col-md-6">
		<div style="text-align: right;">
			<span class="edit-button-<?= $ctr ?> edit-button">
                <a href="javascript:editSelectedInvoiceItem(<?= $ctr ?>)" class="selectedBtns" ><i class="fa fa-edit"></i> Edit</a>
            </span>
            <span class="save-button-<?= $ctr ?> save-button hidden">
                <a href="javascript:saveSelectedInvoiceItem(<?= $ctr ?>)" class="selectedBtns" ><i class="fa fa-save"></i> Save</a>
            </span>
            <span class="remove-button-<?= $ctr ?> remove-button">
                <a href="javascript:removeSelectedInvoiceItem(<?= $ctr ?>)" class="selectedBtns" ><i class="fa fa-trash"></i> Remove</a>
            </span>	
		</div>
	</div>
</div>
...
<div class="row inserted-item-in-list-<?= $ctr ?> selectedItemListDesign selectedItemInInputbox-<?= $ctr?> hidden">
	<?php if($type == '0'): ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-id-in-list-<?= $ctr ?>" value="0-<?= $services_id ?>" name="InvoiceDetail[description][]" class="inputForm form-control partsservicesName" />		
		</div>
	...
	<?php else: ?>
		...
		<div class="col-md-3">
			...
			<input type="hidden" id="parts-services-id-in-list-<?= $ctr ?>" value="1-<?= $parts_id ?>" name="InvoiceDetail[description][]" class="inputForm form-control partsservicesName" />		
		</div>
	
	<?php endif; ?>

	<div class="col-md-3">
		<input type="text" id="parts-services-qty-in-list-<?= $ctr ?>" value="<?= $quantity ?>" name="InvoiceDetail[quantity][]" onchange="updateSelectedSubtotalInvoice(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control partsservicesQty" />	
	</div>

	<div class="col-md-3">
		<input type="text" id="parts-services-price-in-list-<?= $ctr ?>" value="<?= $unit_price ?>" name="InvoiceDetail[unit_price][]" onchange="updateSelectedSubtotalInvoice(<?= $ctr ?>)" class="inputForm inputboxTotalAlignment form-control partsservicesPrice" />	
	</div>
